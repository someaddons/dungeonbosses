...
public class Configuration {
    ...
    public void load() {
        final Path configPath = FabricLoader.getInstance().getConfigDir().normalize().resolve(BrutalBosses.MOD_ID + ".json");
        final File config = configPath.toFile();

        if (!config.exists()) {
            BrutalBosses.LOGGER.warn("Config not found, recreating default");
            save();
        } else {
            try {
                commonConfig.deserialize(gson.fromJson(Files.newBufferedReader(configPath), JsonObject.class));
            } catch (Exception e) {
                BrutalBosses.LOGGER.error("Could not read config from:" + configPath, e);
            }
        }
    }
    ...
    public void save() {
        ...
        try {
            ...
            BrutalBosses.LOGGER.error("Could not write config to:" + configPath, e);
        }
    }
