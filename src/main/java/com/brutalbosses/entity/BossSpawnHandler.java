...
public class BossSpawnHandler
{
    ...
    public static void spawnBoss(final ServerLevelAccessor world, final BlockPos pos, final BossType bossType, final RandomizableContainerBlockEntity chest)
    {
        try
        {
            ...
            if (chest != null && ((BossCapEntity) boss).getBossCap().getLootTable() == null)
            {
            ...
            if (!boss.isRemoved())
            {
                ...
                {
                    if (passenger instanceof BossCapEntity passengerBoss && passengerBoss.getBossCap() != null)
                    {
                        passengerBoss.getBossCap().setSpawnPos(spawnPos);
                    }
