...
{
    public static ResourceLocation ID = ResourceLocation.tryParse("brutalbosses:changegear");
        ...
        public IAIParams parse(final JsonObject jsonElement)
        {
            ...
            if (jsonElement.has(GEAR))
            {
                ...
                for (Map.Entry<String, JsonElement> data : gear.entrySet())
                {
                    ...
                    try
                    {
                        final ItemStack stack = ItemStack.CODEC.parse(NbtOps.INSTANCE, TagParser.parseTag(data.getValue().getAsString())).getOrThrow();
