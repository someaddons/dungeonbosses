...
{
    public static ResourceLocation ID = ResourceLocation.tryParse("brutalbosses:jumpattack");
        ...
        public IAIParams parse(final JsonObject jsonElement)
        {
            ...
            if (jsonElement.has(POTIONS))
            {
                ...
                for (Map.Entry<String, JsonElement> data : jsonElement.get(POTIONS).getAsJsonObject().entrySet())
                {
                    ...
                    potions.add(new PotionData(BuiltInRegistries.MOB_EFFECT.getHolder(ResourceLocation.tryParse(data.getKey())).get(),
