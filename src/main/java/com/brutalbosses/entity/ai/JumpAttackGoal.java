        ...
        public IAIParams parse(final JsonObject jsonElement)
        {
            ...
            if (jsonElement.has(POTIONS))
            {
                ...
                for (Map.Entry<String, JsonElement> data : jsonElement.get(POTIONS).getAsJsonObject().entrySet())
                {
                    ...
                    {
                        particleType = ForgeRegistries.PARTICLE_TYPES.getValue(ResourceLocation.tryParse(potionData.get(POTIONPARTICLE).getAsString()));
                    }

                    potions.add(new PotionData(ForgeRegistries.MOB_EFFECTS.getValue(ResourceLocation.tryParse(data.getKey())),
